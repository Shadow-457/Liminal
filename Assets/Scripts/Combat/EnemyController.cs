...
public class EnemyController : MonoBehaviour, IDamageable, IReadOnlyHealth
{
    ...
    [Tooltip("Only used if the bar's auto height is disabled. The bar normally hugs the TOP of the model automatically.")]
    ...
    [Tooltip("Gap between the very top of the model and the health bar (metres). Raise if it clips, lower to bring it closer.")]
    public float healthBarGap = 0.15f;
    ...
    [Header("AI - Range")]
    [Tooltip("SINGLE control for how close the enemy must be to hit you. Drives attack/melee range. Even if this is high, it can never hit beyond this.")]
    [Range(0.3f, 6f)] public float meleeRange = 0.9f;
    ...
    void Awake()
    {
        ...
        // The single "Range" knob drives both the combat gate and the hit reach.
        attackRange = meleeRange;
        aiFireRange = meleeRange + 0.2f;
        ...
        // It auto-hugs the top of the model's bounds, so the height tracks any resizing.
        var bar = HealthBarUI.CreateWorld(this, transform, healthBarHeight, healthBarSize);
        if (bar != null) bar.boundsGap = healthBarGap;
    }
    ...
    void Update()
    {
        if (_dead || !aiEnabled || !GameManager.IsPlaying) return;
        UpdateAI();
    }
    ...
    private void ResolveTarget()
    {
        ...
        if (_aiTargetHealth == null)
            _aiTargetHealth = FindAnyObjectByType<PlayerHealth>();
