...
public class PlayerHealth : MonoBehaviour, IDamageable, IReadOnlyHealth
{
    ...
    void Update()
    {
        if (_dead)
        {
            // Respawn after a short delay once the respawn key is pressed (unscaled, because
            // Time.time is frozen while the game is paused on the death screen).
            if (Time.unscaledTime - _diedAt >= respawnDelay && Input.GetKeyDown(respawnKey))
                Respawn();
            return;
        }
    ...
    private void TriggerDeath()
    {
        ...
        // Freeze EVERYTHING: gameplay gate off + timeScale 0 + black "YOU DIED" screen.
