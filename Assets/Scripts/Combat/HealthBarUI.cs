...
public class HealthBarUI : MonoBehaviour
{
    ...
    [Tooltip("Height above the tracked object's origin the bar floats at. " +
             "Only used when autoHeightFromBounds is OFF.")]
    public float worldHeight = 2.2f;
    [Tooltip("World-space size of the bar, in metres.")]
    public Vector2 worldSize = new Vector2(2f, 0.24f);
    [Tooltip("World bar: keep it floating just above the TOP of the target's visible model, " +
             "and follow it automatically when the model is scaled/resized.")]
    public bool autoHeightFromBounds = true;
    [Tooltip("Gap between the top of the model and the bar's centre when autoHeightFromBounds is on (world metres).")]
    public float boundsGap = 0.15f;
    ...
    private Renderer[] _renderCache;   // for autoHeightFromBounds
...
private void LateUpdate()
    ...
    private void Billboard()
    {
        ...
        // With autoHeightFromBounds we sit the bar just above the model's visible top
        // every frame, so it hugs small cubes and big models alike and follows resizing.
        Vector3 basePos = _billboardTarget != null
            ? (_world && autoHeightFromBounds ? TopPoint() : _billboardTarget.position + _billboardOffset)
            : transform.position;
    ...
    // A point just above the target's combined renderer bounds (in world space).
    // Falls back to the manual offset if the target has no renderers.
    private Vector3 TopPoint()
    {
        if (_renderCache == null || _renderCache.Length == 0)
        {
            if (_billboardTarget == null) return _billboardOffset;
            _renderCache = _billboardTarget.GetComponentsInChildren<Renderer>(true);
        }

        if (_renderCache.Length == 0)
            return _billboardTarget.position + _billboardOffset;

        Bounds b = _renderCache[0].bounds;
        for (int i = 1; i < _renderCache.Length; i++) b.Encapsulate(_renderCache[i].bounds);

        // Keep a small, size-relative gap so the bar never sinks into the model.
        float gap = Mathf.Max(boundsGap, b.size.y * 0.04f);
        return new Vector3(b.center.x, b.max.y + gap, b.center.z);
    }
